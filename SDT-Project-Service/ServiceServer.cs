...
namespace SDT_Project_Service
{
    ...
    public class ServiceServer : IServiceServer
    {
        ...
        public double GetUserBalance(uint id)
        {
            StringBuilder strings = new StringBuilder();
            ConsoleColor consoleColor = ConsoleColor.White;
            connection.Open();
            ...
            if (reader.HasRows && reader.Read())
            {
                ...
                consoleColor = ConsoleColor.Green;
                strings.AppendLine($"Пользователь под идентификатором {id} запросил данные о текущем балансе:\n ID: {id};\n Баланс: {GetUserTypeInString((uint)result)}.");
            }
            else
            {
                consoleColor = ConsoleColor.DarkYellow;
                strings.AppendLine($"Пользователь под идентификатором {id} не получил ответ на запрос своего баланса.");
            }
        ...
        public void SetUserBalance(int id, double price)
        {
            StringBuilder strings = new StringBuilder();
            ConsoleColor consoleColor = ConsoleColor.White;
            connection.Open();

            string sql = $"UPDATE `users` SET `Balance` = {price} WHERE `ID` = {id}";
            MySqlCommand command = new MySqlCommand(sql, connection);
            MySqlDataReader reader = command?.ExecuteReader();
